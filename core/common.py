...
def convolutional(input_layer, filters_shape, downsample=False, activate=True, bn=True, activate_type='leaky'):
    ...
    if activate == True:
        if activate_type == "leaky":
            ...
            conv = tf.keras.layers.Lambda(mish)(conv)
    return conv
...
def route_group(input_layer, groups, group_id):
    ...
    return tf.keras.layers.Lambda(lambda x: tf.split(x, num_or_size_splits=groups, axis=-1)[group_id])(input_layer)

def upsample(input_layer):
    return tf.keras.layers.UpSampling2D(size=(2, 2), interpolation='bilinear')(input_layer)
